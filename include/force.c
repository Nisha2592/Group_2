...
void force(mdsys_t *sys, int my_rank, int comm_size, MPI_Comm comm) {
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &sys->comm_size);
    ...
    c6 *= 4.0 * sys->epsilon;
     ...
     for (ii=0; ii < (sys->natoms-1); ii += sys->comm_size) {
          int i,j;
          double rx1, ry1, rz1;
          ...
          for(j=i+1; j < (sys->natoms); ++j) {
           ...
           // Calculate distance between atoms i and j
            double rx = pbc(rx1 - sys->rx[j], 0.5 * sys->box, sys->box);
            double ry = pbc(ry1 - sys->ry[j], 0.5 * sys->box, sys->box);
            double rz = pbc(rz1 - sys->rz[j], 0.5 * sys->box, sys->box);
            double rsq = rx * rx + ry * ry + rz * rz;

            // Apply cutoff distance
            if (rsq > 1e-12 && rsq < rcsq) {
                double rinv = 1.0 / rsq;
